...
import {
  isMriPluginMessage,
  type MriPluginGuestMessage,
  type MriPluginHostMessage,
  type MriPluginUiConfig,
} from './types'
 ...
 *
 * Diferente da cópia do mri_Qweather, este bridge também carrega o `uiConfig`
 * — o host já envia o campo em `init` e `theme-changed`, e sem ele o plugin
 * ignora radius/fonte/cores de status vindos do painel /uiconfig do ox_lib.
 */
export function usePluginBridgeGuest(opts: Options = {}) {
  ...
  useEffect(() => {
    ...
    // Sem este `ready` o host mostra "Plugin indisponivel" depois de 10s.
