...
local function CrouchKeyPressed()
    ...
    if IsPauseMenuActive() or IsNuiFocused() or LocalPlayer.state.invOpen then
        return
    end
...
if Config.CrouchEnabled then
    if Config.CrouchKeybindEnabled then
        RegisterKeyMapping('+crouch', locale('register_crouch'), 'keyboard', Config.CrouchKeybind)
...
if Config.CrawlEnabled then
    if Config.CrawlKeybindEnabled then
        RegisterKeyMapping('+crawl', locale('register_crawl'), 'keyboard', Config.CrawlKeybind)
